...
public class OJDBCExtractor extends OAbstractExtractor {
	...
	@Override
	public void configure(OETLProcessor iProcessor, ODocument iConfiguration,
			OBasicCommandContext iContext) {
		...
		try {
			...
			throw new OConfigurationException("[JDBC extractor] JDBC Driver "
		...
		try {
			...
			throw new OConfigurationException(
					"[JDBC extractor] error on connecting to JDBC url '" + url
							+ "' using user '" + userName
	...
	public void begin() {
		try {
			...
			throw new OExtractorException(
					"[JDBC extractor] error on executing query '" + query + "'",
					e);
		}
	}
		...
		try {
			...
			throw new OExtractorException(
					"[JDBC extractor] error on moving forward in resultset of query '"
							+ query + "'. Previous position was " + current, e);
		}
	}
			...
			if (!didNext) {
				if (!rs.next())
					throw new NoSuchElementException(
							"[JDBC extractor] previous position was " + current);
			}
			didNext = false;
	...
	public ODocument getConfiguration() {
		return new ODocument()
				.fromJSON("{parameters:[{driver:{optional:false,description:'JDBC Driver class'}},"
