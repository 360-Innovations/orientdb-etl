...
public class OOrientDBLoader extends OAbstractLoader implements OLoader {
			...
			if (dbAutoCreateProperties) {
				if (dbType == DB_TYPE.DOCUMENT) {
					if (input instanceof ODocument) {
						...
						final ODatabaseDocumentTx documentDatabase = pipeline
								.getDocumentDatabase();
						final OClass cls;
						if (className != null)
					...
					if (input instanceof OrientElement) {
						...
						final String clsName = className != null ? className
								: (element instanceof OrientVertex ? ((OrientVertex) element)
										.getLabel() : ((OrientEdge) element)
										.getLabel());
						if (clsName != null)
							cls = getOrCreateClass(clsName,
									element.getBaseClassName());
						else
							throw new IllegalArgumentException(
									"No class defined on graph element: "
											+ input);
	...
	@Override
	public void configure(final OETLProcessor iProcessor,
			final ODocument iConfiguration, final OBasicCommandContext iContext) {
		...
		if (iConfiguration.containsField("dbType"))
			dbType = DB_TYPE.valueOf(iConfiguration.field("dbType").toString()
					.toUpperCase());
		...
		if (iConfiguration.containsField("dbAutoDropIfExists"))
			dbAutoDropIfExists = (Boolean) iConfiguration
					.field("dbAutoDropIfExists");
		if (iConfiguration.containsField("dbAutoCreateProperties"))
			dbAutoCreateProperties = (Boolean) iConfiguration
					.field("dbAutoCreateProperties");
		if (iConfiguration.containsField("inMemory"))
			inMemory = (Boolean) iConfiguration
					.field("inMemory");
		...
		switch (dbType) {
		case DOCUMENT:
			final ODatabaseDocumentTx documentDatabase = new ODatabaseDocumentTx(
					dbURL);
			if (dbAutoDropIfExists && documentDatabase.exists()) {
				log(OETLProcessor.LOG_LEVELS.INFO,
						"Dropping existent database '%s'...", dbURL);
				documentDatabase.open(dbUser, dbPassword);
				documentDatabase.drop();
			}

			if (documentDatabase.exists()) {
				log(OETLProcessor.LOG_LEVELS.DEBUG, "Opening database '%s'...",
						dbURL);
				documentDatabase.open(dbUser, dbPassword);
			} else if (dbAutoCreate) {
				documentDatabase.create();
			} else
				throw new IllegalArgumentException("Database '" + dbURL
						+ "' not exists and 'dbAutoCreate' setting is false");
		...
		case GRAPH:
			...
			final OrientBaseGraph graphDatabase = tx ? factory.getTx()
					: factory.getNoTx();
	...
	protected void createProperty(final OClass cls, final String f,
			final Object fValue) {
		if (fValue != null) {
			...
			log(OETLProcessor.LOG_LEVELS.DEBUG,
					"created property [%s.%s] of type [%s]", cls.getName(), f,
					fType);
		}
	}
	...
	protected synchronized ODatabaseDocumentTx init() {
		...
		if (className != null) {
			...
			log(OETLProcessor.LOG_LEVELS.DEBUG,
					"%s: found %d %s in class '%s'", getName(),
					schemaClass.count(), getUnit(), className);
		}

		if (classes != null) {
			for (ODocument cls : classes) {
				schemaClass = getOrCreateClass((String) cls.field("name"),
						(String) cls.field("extends"));
				...
				if (fields != null) {
					for (ODocument field : fields) {
						OProperty property = schemaClass.createProperty(
								(String) field.field("name"),
								OType.valueOf((String) field.field("type")));
		...
		if (indexes != null) {
			for (ODocument idx : indexes) {
				...
				if (idxName != null) {
					index = documentDatabase.getMetadata().getIndexManager()
							.getIndex(idxName);
					if (index != null)
						// ALREADY EXISTS
						continue;
				...
				for (int f = 0; f < fields.length; ++f) {
					...
					if (!cls.existsProperty(fieldNameParts[0])) {
						...
						if (fieldNameParts.length < 2)
							throw new OConfigurationException(
									"Index field type missed in OrientDB Loader for field '"
											+ fieldName + "'");

						final String fieldType = fieldNameParts[1]
								.toUpperCase();
	...
	protected OClass getOrCreateClass(final String iClassName,
		...
		if (dbType == DB_TYPE.DOCUMENT) {
			...
			final ODatabaseDocumentTx documentDatabase = pipeline
					.getDocumentDatabase();
			if (documentDatabase.getMetadata().getSchema()
					.existsClass(iClassName))
				cls = documentDatabase.getMetadata().getSchema()
						.getClass(iClassName);
			else {
				if (iSuperClass != null) {
					final OClass superClass = documentDatabase.getMetadata()
							.getSchema().getClass(iSuperClass);
					if (superClass == null)
						throw new OLoaderException("Cannot find super class '"
								+ iSuperClass + "'");

					cls = documentDatabase.getMetadata().getSchema()
							.createClass(iClassName, superClass);
					log(OETLProcessor.LOG_LEVELS.DEBUG,
							"- OrientDBLoader: created class '%s' extends '%s'",
							iClassName, iSuperClass);
				} else {
					cls = documentDatabase.getMetadata().getSchema()
							.createClass(iClassName);
					log(OETLProcessor.LOG_LEVELS.DEBUG,
							"- OrientDBLoader: created class '%s'", iClassName);
				}
			}
		} else {
			// GRAPH
			...
			if (cls == null) {
				...
				if (iSuperClass != null) {
					final OClass superClass = graphDatabase.getRawGraph()
							.getMetadata().getSchema().getClass(iSuperClass);
					if (superClass == null)
						throw new OLoaderException("Cannot find super class '"
								+ iSuperClass + "'");

					if (graphDatabase.getVertexBaseType().isSuperClassOf(
							superClass)) {
						// VERTEX
						...
						// EDGE
						cls = graphDatabase.createEdgeType(iClassName,
								superClass);
						log(OETLProcessor.LOG_LEVELS.DEBUG,
					...
					cls = graphDatabase.createVertexType(iClassName);
					log(OETLProcessor.LOG_LEVELS.DEBUG,
							"- OrientDBLoader: created vertex class '%s'",
							iClassName);
				}
			}
		}
	...
	private String transformFieldName(String f) {
		...
		if (Character.isDigit(first))
			return "field" + Character.toUpperCase(first)
					+ (f.length() > 1 ? f.substring(1) : "");
		return null;
	}
}
