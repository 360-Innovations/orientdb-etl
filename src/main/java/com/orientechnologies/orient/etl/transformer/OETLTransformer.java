...
public class OETLTransformer extends OAbstractLookupTransformer {
  ...
  public void configure(OETLProcessor iProcessor, final ODocument iConfiguration, OBasicCommandContext iContext) {
    ...
    fileName = iConfiguration.field("fileName");
  ...
  public Object executeTransform(final Object input) {
    if (fileName == null || fileName.length() == 0) {
      throw new OConfigurationException("fileName must be specified for etl");
    }
    ...
    if (parameters != null) {
      ...
      for (String name : parameters.fieldNames()) {
        if (!sqlFilters.containsKey(name)) {
          ...
          sqlFilters.put(name, new OSQLFilter(formula, context, null));
        }

        OSQLFilter sqlFilter = sqlFilters.get(name);
        String value = sqlFilter.evaluate((ODocument) input, null, context).toString();

        params.add('-' + name + '=' + value);
      }
    }
    
    for (Map.Entry<String, Object> entries : iContext.getVariables().entrySet()) {
      params.add('-' + entries.getKey() + '=' + entries.getValue());
    }

    final List<Object> results = OETLProcessor.executeSubETL(params.toArray(new String[] {}));
